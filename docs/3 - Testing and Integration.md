...
|AUTOINT3|Check if the low power mode is enabled, if so force delay.||||If the low power mode is enabled, then make sure that the maximum interval for battery charging is used to maximise charging.|Y|
|AUTOINT4|Check if the interval control is disabled, if so force no delay.||||If the interval control is disabled, then test that there is no interval delay.|N - cannot automate this since interval enable/disable is hardcoded using macro define.|
