...
#define WATCHDOG_LOOP_HEARTBEAT_PERIOD                  1000 /*!< Wachdog heartbeat period in loop() (ms). */
 ...
 * @{
 */
#define LOW_POWER_MODE_ACTIVE                           0 /*!< Whether the low power mode is active (0 is no, 1 is yes). */
...
#define TRANSMIT_ENABLED                                1 /*!< Whether the satellite can trasmit (0 is no, 1 is yes). */
/**
 * @}
 */
 ...
 * |Callsign (string).|0x0006|0x0026|21|
 * |Total|||27|
 * @{
 */
 ...
 * 
 * @todo !IMPORTANT! which references are used for these pins (18/01/2020)
 * @todo please specify the direction of the RADIO_NSS, RADIO_BUSY and RADIO_NRST defines.
 * @todo Julian -> Please check this.
 * 
 * @brief
 * |Description|Pin Code Used|Referenced Pin Code (?)|Mode|Direction|
 * |--|--|--|--|--|
 ...
 * |Radio BUSY pin.|6|PD6|DIGITAL|OUT (?)|
 * |Radio NRST pin.|NC|?|?|?|
 * 
 * @{
 */
 ...
 * 
 * @todo Jan -> What does MCU_TEMP_COEFFICIENT do?
 * 
 * @{
 */
